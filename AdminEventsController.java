...
public class AdminEventsController {
    ...
    @RequestMapping(value = {"/admin/collections/{titreEvenement}"}, method = RequestMethod.GET)
    public ModelAndView viewObjectsCollections(@PathVariable(value = "titreEvenement") String titreEvenement,
                                               HttpSession httpSession) throws IOException, BiffException {
        if (Helper.isConnect(httpSession)) {
            HashMap<String, Object> model = new HashMap<>();
            model.put("evenement", titreEvenement);
            model.put("titreEvenement", titreEvenement);
